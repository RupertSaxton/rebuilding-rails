...
class ActionViewTest < Minitest::Test
  ...
  def test_render_with_helper
    ...
    context = ActionView::Base.new var: "var value"
    assert_equal "<a href=\"/url\">title</a>", template.render(context) { "yielded content"}
  end
  ...
  def test_render
    ...
    assert_match "<h1>Choc chip muffins</h1>", response.body
  ...
  def test_render_index
    ...
    assert_match "<h1>The Muffin Blog</h1>", response.body
    assert_match "<html>", response.body
  end
end
